...
public class SlidingWindow {
    ...
    /** 队列的总长度 */
    private final int timeSlicesSize;
    ...
    public SlidingWindow(int duration, int threshold) {
        ...
        // 保证存储在至少两个window
        this.timeSlicesSize = windowSize * 2;
    ...
    private void reset() {
        ...
        //窗口个数
        AtomicInteger[] localTimeSlices = new AtomicInteger[timeSlicesSize];
    ...
    private int locationIndex() {
        ...
        return (int) (((now - beginTimestamp) / timeMillisPerSlice) % timeSlicesSize);
    }
    ...
    public synchronized boolean addCount(int count) {
        ...
        for (int i = 1; i < windowSize; i++) {
            sum += timeSlices[(index - i + timeSlicesSize) % timeSlicesSize].get();
        }
